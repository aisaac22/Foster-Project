/**
 * CSV ingest pipeline (batched). Shared between the CLI (scripts/ingest.ts)
 * and the admin CSV-upload modal — both call `ingest()` directly with CSV
 * text in memory, no file-path assumptions.
 *
 * Validation and parsing are unchanged from the original CLI-only version.
 * Rows are inserted in chunks of ~500 via multi-row VALUES, and every lookup
 * (counties, provider existence, episode ids) is resolved once into an
 * in-memory Map instead of one query per row.
 */
